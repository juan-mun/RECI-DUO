...
import {
  FileText, MapPin, Calendar as CalendarIcon, Clock, Send, Package,
...
export default function RecolectoraSolicitudes() {
  const { user } = useAuth();
  ...
  const { data: solicitudes = [], isLoading } = useQuery({
    queryKey: ['solicitudes-disponibles-all'],
    queryFn: async () => {
  ...
  // Fetch generadora profiles for all solicitudes
  const userIds = useMemo(() => [...new Set(solicitudes.map((s: any) => s.user_id))], [solicitudes]);
  const { data: profiles = [] } = useQuery({
    queryKey: ['generadora-profiles', userIds],
    queryFn: async () => {
  ...
  const enviarOferta = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['solicitudes-disponibles-all'] });
      queryClient.invalidateQueries({ queryKey: ['mis-ofertas-detail'] });
      queryClient.invalidateQueries({ queryKey: ['oferta-counts'] });
      ...
      {filteredSolicitudes.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-md">
              No hay solicitudes disponibles en tu zona para tus tipos de residuo. Revisa más tarde o amplía tu zona de cobertura.
            </p>
            {hasActiveFilters && (
        ...
        <div className="grid gap-4">
          {filteredSolicitudes.map((sol: any) => {
            const yaOfertada = misOfertasMap[sol.id];
